-- supabase/migrations/20260901000006_security_hardening.sql
-- Security Hardening:
-- 1. Search Path Hijacking mitigation on SECURITY DEFINER functions
-- 2. Lockdown public SELECT access on user_feedback (Write-Only for Anonymous Users)
-- 3. Constraint-based payload size limits against DoS / storage exhaustion

-- ====================================================================
-- 1. HARDEN SECURITY DEFINER FUNCTIONS (Explicit Search Path)
-- Prevents CWE-426 Untrusted Search Path / Search Path Hijacking attacks
-- ====================================================================
...
BEGIN
    ...
    -- 5. Required and valid source URL (Strict protocol check)
...
BEGIN
    ...
    -- City-level data_confidence rollup
    SELECT 
        CASE 
...
-- ====================================================================
-- 2. LOCK DOWN USER_FEEDBACK RLS POLICIES (Prevent Feedback Snooping)
-- Anonymous users can write feedback, but cannot read submissions of other users
-- ====================================================================

-- Drop permissive public read policy
DROP POLICY IF EXISTS "Allow public read for user_feedback" ON user_feedback;

-- Allow only authenticated administrators or service_role to read feedback
CREATE POLICY "Allow authenticated read for user_feedback"
    ON user_feedback FOR SELECT
    TO authenticated
    USING (true);

-- Ensure anon role cannot read user_feedback
CREATE POLICY "Deny anon read to user_feedback"
    ON user_feedback FOR SELECT
    TO anon
    USING (false);
...
-- ====================================================================
-- 3. PAYLOAD BOUNDS CONSTRAINTS (Anti-DoS Protection)
-- ====================================================================

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'check_submission_note_len'
    ) THEN
        ALTER TABLE submissions ADD CONSTRAINT check_submission_note_len 
            CHECK (note IS NULL OR length(note) <= 1000);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'check_submission_evidence_url_len'
    ) THEN
        ALTER TABLE submissions ADD CONSTRAINT check_submission_evidence_url_len 
            CHECK (evidence_url IS NULL OR length(evidence_url) <= 2048);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'check_feedback_message_len'
    ) THEN
        ALTER TABLE user_feedback ADD CONSTRAINT check_feedback_message_len 
